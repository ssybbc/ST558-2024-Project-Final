...
It looks like through 5-fold cross validation, the best model (judging by the least mean squared error) is with the most tree depth. This is reasonable considering we have so many predictors to look at. We could use the *select_best* function to extract the model with the lowest rmse.
...
```{r}
diabetes_rf_best_params <- select_best(diabetes_sample_fit, "mn_log_loss")
